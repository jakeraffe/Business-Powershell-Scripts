# This script pulls from a textfile which contains pc names and checks against AD for machine information.
#
#
...
# --- START FUNCTIONS --- #
#
# This is the first of two functions that allows us to open a file from a location
# Requires a text file extension
function open_Text_File{
    ...
    if ($OpenFileDialog.ShowDialog() -eq "OK"){
            $textbox_BrowseForFile.Text = $OpenFileDialog.FileName   
    }
}
...
$LabelLabel_BrowseForFile = New-Object System.Windows.Forms.Label
    $LabelLabel_BrowseForFile.Location = New-Object System.Drawing.Size(8,$Y)
    $LabelLabel_BrowseForFile.Size = $LabelSize_universal
    $LabelLabel_BrowseForFile.TextAlign = $TextAlign_universal
    $LabelLabel_BrowseForFile.Text = "Open Text File Location"
        $Form_MachineInfo.Controls.Add($LabelLabel_BrowseForFile)
 
# Adding a textbox for the path to show
$textbox_BrowseForFile = New-Object System.Windows.Forms.TextBox
    $textbox_BrowseForFile.Location = New-Object System.Drawing.Size(192,$Y)
    $textbox_BrowseForFile.Size = $TextBoxSize_universal
        $Form_MachineInfo.Controls.Add($textbox_BrowseForFile)

# add a button
$button_BrowseForFile = New-Object System.Windows.Forms.Button
    $button_BrowseForFile.Location = New-Object System.Drawing.Size(448,$Y)
    $button_BrowseForFile.Size = $buttonSize_universal
    $button_BrowseForFile.Text = "Browse"
    $button_BrowseForFile.Add_Click({open_Text_File})
        $Form_MachineInfo.Controls.Add($button_BrowseForFile)
...
function script_Main{
    ...
    if ($textbox_BrowseForCSV.Text -ne ""){
        ...
        $names = Get-content $textbox_BrowseForFile.Text
        

        # Now we import the path for Configuration Manager and change directories to the CMSite
        Import-Module ('C:\ConfigMgr\bin\ConfigurationManager.psd1') -Scope Global
        Set-Location ((get-psdrive -PSProvider CMSite).Name+":")

        #just makes it easier to use variables for these strings
        $Pinged = "Pinged"
        $Down = "Down"

        #Set to the current date only to initialize
        $ADTime = Get-Date

        # Main Array
        Write-Host "             Loading..Please press 'ok' to continue           "
        $outline = @(
            foreach ($name in $names){
        
                #  ------Error checking ------
                try{
                    $OpSys = (Get-ADComputer -identity $name -Properties OperatingSystem | FT OperatingSystem -HideTableHeaders | Out-String).Trim()
                    $ADTime = (Get-ADComputer -identity $name -Properties LastLogonDate | FT LastLogonDate -HideTableHeaders | Out-String).Trim()
                    $SCCM = (Get-CMDevice -Name "$name" | Select LastLogonUser | FT -HideTableHeaders | Out-String).Trim()
                }
                catch{
                    $ADTime = "No info found"
                    $OpSys = "OS Not found"
                    $SCCM = "Not in SCCM"
                }
                #  ------End error checking ------
                ...
                if (Test-Connection -ComputerName $name -Count 3 -ErrorAction SilentlyContinue){
                    ...
                    #Gets IP for the current index
                    $ipv4 = (Test-Connection $name -Count 1 -Delay 2| Select IPV4Address | FT -HideTableHeaders | Out-String).Trim()
                    ...
                    [pscustomobject]@{
                        ...
                        OS = $OpSys
                        LastLogon_AD = $ADTime 
                        In_AD = if ($ipv4 -eq "No connection to machine" -and $OpSys -eq "OS Not found" -and $ADTime -eq "No info found"){"No"} else {"Yes"}
                        LastLogon_SCCM = $SCCM
                    }# -- End Hash table --
                    # Console output for readability 
                    #Write-Host $name ":" $Pinged ":" $ipv4 ":" $OpSys ":" $ADTime ":" $SCCM
                }
                ...
                # If a connection Does not Exist....
                    ...
                    [pscustomobject]@{
                        ...
                        OS = $OpSys
                        LastLogon_AD = $ADTime 
                        In_AD = if ($OpSys -eq "OS Not found" -and $ADTime -eq "No info found"){"No"} else {"Yes"}
                        LastLogon_SCCM = $SCCM
                    } # -- End Hash table --
                    # Console output for readability 
                    #Write-Host $name ":" $Pinged ":" $ipv4 ":" $OpSys ":" $ADTime ":" $SCCM
       ...
       # Write-Host "                Completed Successfully           "
        #Set-Location ((Get-PSDrive -PSProvider FileSystem).Name+":")
        $outline| Export-Csv -Path $textbox_BrowseForCSV.Text -NoTypeInformation  
        #$outline| Export-Csv -Path C:\CheckComputerStatus\computer-Ping.csv -NoTypeInformation  
