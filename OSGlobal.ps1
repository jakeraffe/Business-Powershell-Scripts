...
function script_Main{
    ...
    if ($textbox_BrowseForCSV.Text -ne ""){
        ...
        $names = Get-ADComputer -Filter {OperatingSystem -notlike '*Server*'} -Properties OperatingSystem, IPv4Address| Select-Object -Property Name, OperatingSystem, IPv4Address
    ...
    Invoke-Item "$folderDirectory\Macro.XLSB"
}
